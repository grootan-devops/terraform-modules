...
resource "aws_iam_role" "eks_vpc_cni" {
  name = "${local.name_prefix}-eks-vpc-cni"
...
resource "aws_iam_role" "efs_csi_node" {
  name = "${local.name_prefix}-efs-csi-node"
...
resource "aws_iam_policy" "efs_csi_node" {
  name        = "${local.name_prefix}-efs-csi-node-policy"
  description = "Permissions for EFS CSI Node to mount and write to file systems"

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid    = "AllowMountAndWriteViaMountTarget"
        Effect = "Allow"
        Action = [
          "elasticfilesystem:ClientMount",
          "elasticfilesystem:ClientWrite"
        ]
        Resource = "arn:aws:elasticfilesystem:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:file-system/*"
        Condition = {
          Bool = {
...
resource "aws_iam_role_policy_attachment" "efs_csi_node" {
  ...
  policy_arn = aws_iam_policy.efs_csi_node.arn
}

resource "aws_iam_role" "s3_csi" {
  name = "${local.name_prefix}-s3-csi"
